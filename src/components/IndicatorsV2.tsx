...
const BarHorizontal: React.FC<BarHorizontalProps> = ({ current, previous, baseline, max }) => {
  const [hovered, setHovered] = useState(false);
  const initialHeight = 12;
  const barHeight = hovered ? 24 : initialHeight;
  const gap = hovered ? 4 : 0;
  const containerHeight = hovered ? barHeight * 3 + gap * 2 : barHeight;

  const toPct = (v: number) => (max === 0 ? 0 : (v / max) * 100);

  // calculate bottom offsets for split bars
  const baseOffset = 0;
  const prevOffset = hovered ? barHeight + gap : 0;
  const currOffset = hovered ? (barHeight + gap) * 2 : 0;
            ...
            <div className="flex flex-wrap gap-6 items-center">
              ...
              <div className="ml-auto flex items-center gap-6">
                ...
                <div className="flex items-center space-x-2">
                  ...
                  <span className="text-sm font-medium">{redDate} (Baseline)</span>
                </div>
              </div>
            </div>
          </CardHeader>
