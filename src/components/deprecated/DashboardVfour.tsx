...
import BoundarySvg from '@/components/BoundarySvg';
            ...
            {selectedTheme && (
                <div className="space-y-4">
                    ...
                    {Object.keys(THEMES[selectedTheme].indicators).map(indicatorKey => (
                        <IndicatorSection key={indicatorKey} themeKey={selectedTheme} data={data} />
                    ))}
                </div>
            )}
        </div>
...
const IndicatorSection = ({ data, themeKey }) => {
    const [expandedIndicator, setExpandedIndicator] = useState(null);

    const handleToggle = (indicatorKey) => {
        setExpandedIndicator(prev =>
            prev?.indicatorKey === indicatorKey ? null : { themeKey, indicatorKey }
        );
    };
        ...
        <div className="space-y-4">
            {Object.keys(THEMES[themeKey].indicators).map(indicatorKey => (
                <IndicatorCard
                    ...
                    data={data}
                    isActive={expandedIndicator?.indicatorKey === indicatorKey}
            ...
            {expandedIndicator && (
                <IndicatorDetailPanel
                    themeKey={expandedIndicator.themeKey}
                    indicatorKey={expandedIndicator.indicatorKey}
                    data={data}
                />
            )}
        </div>
...
const IndicatorCard = ({ themeKey, indicatorKey, data, isActive, onToggle }) => {
    const indicatorName = THEMES[themeKey]?.indicators[indicatorKey] || 'Unknown';

    const performance = useMemo(() => {
        if (!data?.length) return { best: null, worst: null };
        const isReversed = ['lowBirthWeight', 'samChildren'].includes(indicatorKey);
        const sorted = [...data].sort((a, b) => {
            const aVal = a[themeKey]?.[indicatorKey] ?? (isReversed ? Infinity : -Infinity);
            const bVal = b[themeKey]?.[indicatorKey] ?? (isReversed ? Infinity : -Infinity);
    ...
    return (
        <Card onClick={() => onToggle(indicatorKey)} className={`cursor-pointer ${isActive ? 'border-blue-500' : ''}`}>
            <CardContent className="pt-4">
                <div className="flex justify-between items-center">
                    <h4 className="font-medium text-sm sm:text-base">{indicatorName}</h4>
                    <Badge variant={isActive ? "default" : "outline"}>
                        {isActive ? "Active" : "View"}
                    </Badge>
                </div>
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 text-xs mt-2 text-muted-foreground">
                    <div><Badge variant="outline" className="border-green-500 text-green-600">Best</Badge> {performance.best?.blockName}</div>
                    <div><Badge variant="destructive">Worst</Badge> {performance.worst?.blockName}</div>
                </div>
            </CardContent>
        </Card>
    );
};
const IndicatorDetailPanel = ({ themeKey, indicatorKey, data }) => {
    const indicatorName = THEMES[themeKey]?.indicators[indicatorKey] || 'Unknown';
    const { desc, sdg } = INDICATOR_DETAILS[indicatorKey] || {};
    ...
    return (
        <Card className="border-t-4 border-blue-500 shadow-sm mt-2">
            <CardContent className="py-4">
                <h4 className="font-semibold text-base mb-2">{indicatorName}</h4>
                <p className="text-xs text-muted-foreground mb-1">
                    <span className="font-medium text-foreground">Description:</span> {desc || 'N/A'}
                </p>
                <p className="text-xs text-muted-foreground mb-3">
                    <span className="font-medium text-foreground">Related SDG:</span> {sdg || 'N/A'}
                </p>

                <div className="grid gap-2 max-h-52 overflow-y-auto pr-1">
                    {sorted.map((block, idx) => {
                        const val = block[themeKey]?.[indicatorKey] ?? 0;
                        return (
                            <div key={idx} className="flex justify-between text-xs items-center">
                                <span className="truncate">{block.blockName} ({block.districtName})</span>
                    ...
                    <Card key={block.sno} className="cursor-pointer hover:border-primary transition-colors" onClick={() => onBlockSelect(block)}>
                        <div className="flex gap-4 p-4 items-center">
                            <div className="w-1/3 flex-shrink-0">
                                <BoundarySvg geoCsvData={geoCsvArray} blockName={block.blockName} />
                            </div>
